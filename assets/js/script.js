(() => {
  ...
  const sectionNames = ["Intro", "Brand & Logo", "Social Media", "Apparel & Print", "About"];
  const state = {
    activeIndex: 0,
    activeVideo: 0,
    heroWord: 0,
  ...
  function setMenu(open, options = {}) {
    ...
    if (menuToggleLabel) menuToggleLabel.textContent = shouldOpen ? "Close menu" : "Open menu";
    if (siteNav) {
  ...
  function updateNavigation() {
    ...
    if (scrollCursorLabel) {
      scrollCursorLabel.textContent = state.activeIndex === panels.length - 1 ? "Scroll back" : "Scroll";
    }
  ...
  function switchPanel(nextIndex, options = {}) {
    ...
    // Update the menu state for the new section immediately, instead of waiting for the video transition to finish.
    navButtons.forEach((button) => {
